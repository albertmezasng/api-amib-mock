...
const resolveRecordServicesByRecordOrRequestId = (recordServices, value) => {
  ...
  const byRecordId = recordServices.filter(
    (record) => Number(record.id) === numericValue,
  );
  if (byRecordId.length > 0) {
    return byRecordId;
  }

  return recordServices.filter(
    (record) => Number(record.requestId) === numericValue,
  );
};

const buildFormTemplateResponse = (record) => {
  const applicableForms =
    Number(record.serviceId) === 1
      ? [
          {
            serviceId: record.id,
...
const buildRequestServiceDetail = (record) => ({
  ...
  documents: (record.documents ?? []).map((document) => ({
    ...
    recordDocumentId:
      document.recordDocumentId ?? document.record_document_id ?? null,
  })),
  statusDescription: record.statusName,
  statusId: record.statusId,
  applicantManagement: record.applicantManagement,
  businessLineDescription:
    record.businessLineName || record.businessLineTypeName,
...
const createRecordServiceEntity = ({
  service,
  id,
  requestId,
  applicantManagement,
  ...
  return {
    ...
    typeDescription:
      service.typeDescription ?? `Servicio ${service.serviceId ?? id}`,
    subtypeId: Number(service.subtypeId ?? service.serviceId ?? 0),
    subtypeName:
      service.subtypeName ?? `Subservicio ${service.serviceId ?? id}`,
    subtypeDescription:
      service.subtypeDescription ?? `Subservicio ${service.serviceId ?? id}`,
    applicantId,
    ...
    statusAbbreviation:
      service.statusAbbreviation ?? DEFAULT_STATUS.abbreviation,
    createDate: now,
    updateDate: now,
    recordServiceTypeId: Number(service.recordServiceTypeId ?? 1),
    documentVersionId: Number(service.documentVersionId ?? 1),
    businessLineTypeName:
      service.businessLineTypeName ??
      `Linea de negocio ${service.businessLineTypeId ?? ""}`.trim(),
    evaluationMethodId: Number(service.evaluationMethodId ?? 0),
    evaluationMethodName:
      service.evaluationMethodName ??
      `Metodo ${service.evaluationMethodId ?? 0}`,
    ...
    institutionTypeName:
      service.institutionTypeName ??
      `Institucion ${service.institutionTypeId ?? 0}`,
...
const filterRecordServices = (records, query) => {
  ...
  return records.filter((record) => {
    const matchesNumericFilters = numericFilters.every(
      ([queryKey, recordKey]) => {
...
router.get("/rule/:id", (req, res) => {
  const db = getConnection();
  const data = db.data;
  const rule = data.validations.find(
    (rule) => rule.id === parseInt(req.params.id, 10),
  );
  // console.log(rule);
  if (rule !== undefined) {
...
router.put("/rule/:id", async (req, res) => {
  ...
  if (rule !== undefined) {
    ...
    db.data.validations.map((rule1) =>
      rule1.id === req.params.id ? rule : rule1,
    );
    await db.write();
...
router.delete("/rule/:id", async (req, res) => {
  ...
  // console.log(rule);
  if (rule !== undefined) {
    const newValidations = db.data.validations.filter(
...
router.patch(
  "record-institution-business-line/record/{recordId}/update-status",
  async (req, res) => {
    res.status(200).json({ statusCode: 200, message: "Deleted sucessfully" });
  },
);
...
router.get(
  "/record-institution-business-line/record/:recordId/document",
  async (req, res) => {
...
router.get(
  "/record-institution-business-line/record/:recordId/document/:recordDocumentId",
  async (req, res) => {
    res.status(200).json({
      statusCode: 200,
      data: {
...
router.get(
  "/record-institution-business-line/record/:recordId/document/:recordDocumentId/rules",
  async (req, res) => {
    return res.status(200).json({
      statusCode: 200,
      data: [
...
router.post("/event/:id/activate-exam", async (req, res) => {
  ...
  res
    .status(200)
    .json({ statusCode: 200, data: { message: "Activated sucessfully" } });
});
...
router.get("/relationship-validate/:applicantId", async (req, res) => {
  ...
  if (applicantId === "2500010") {
    return res.status(409).json({
      ...
      label: "App error",
      message:
        "El sustentante esta vinculado a otra institucion. Debe solicitar su baja en dicha institucion y posteriormente solicitar nuevamente el servicio.",
...
router.put("/request/:requestId", async (req, res) => {
  try {
    ...
    const requestStatusId = Number(
      req.body?.requestStatusId ?? req.body?.requestTypeId,
    );
    ...
    const requestStatusDescription =
      REQUEST_STATUS_CATALOG[requestStatusId] ||
      `Request status ${requestStatusId}`;
    const updatedAt = new Date().toISOString();
    const updatedRecords = recordServices
...
router.get("/record-service/document/:recordServiceId", async (req, res) => {
  const db = getConnection();
  const recordServices = getRecordServices(db);
  const record = recordServices.find(
    (item) => Number(item.id) === Number(req.params.recordServiceId),
  );
...
router.get("/record-service/:recordId", async (req, res) => {
  ...
  const recordServices = getRecordServices(db);
  const record = recordServices.find(
    (item) => item.id === Number(req.params.recordId),
  );
...
router.get(
  "/record-service/:recordId/document/:documentId",
  async (req, res) => {
    const db = getConnection();
    const recordServices = getRecordServices(db);
    const record = recordServices.find(
      (item) => Number(item.id) === Number(req.params.recordId),
    );
    const document = record?.documents?.find(
      (item) =>
        Number(item.record_document_id ?? item.id) ===
...
router.post("/record-service/:recordId/document", async (req, res) => {
  ...
  const documentId =
    Number(req.body?.documentId) || Math.floor(Math.random() * 1000);
  const fileId =
    req.body?.filePath ||
    `doc-${req.params.recordId}-${documentId}-${Date.now()}.pdf`;
  const document = record.documents?.find(
    (item) =>
      Number(item.id) === documentId || Number(item.documentId) === documentId,
  );
...
router.patch(
  "/record-service/:recordId/document/:documentId/confirm",
  async (req, res) => {
    const db = getConnection();
    const recordServices = getRecordServices(db);
    const record = recordServices.find(
    ...
    const document = record?.documents?.find(
      (item) =>
        Number(item.record_document_id) === Number(req.params.documentId),
    );
...
router.patch(
  "/record-service/:documentId/document/:recordId/confirm",
  async (req, res) => {
    const db = getConnection();
    const recordServices = getRecordServices(db);
    const record = recordServices.find(
...
router.get(
  "/form-templates/by-record-service/:recordServiceId",
  async (req, res) => {
    const db = getConnection();
    const recordServices = getRecordServices(db);
    const resolvedRecords = resolveRecordServicesByRecordOrRequestId(
      recordServices,
      req.params.recordServiceId,
    );
...
router.post("/form-answers", async (req, res) => {
  const db = getConnection();
  const recordServices = getRecordServices(db);
  const recordServiceId = Number(
    req.body?.recordServiceId || req.body?.serviceId,
  );
  const record = recordServices.find(
    (item) => Number(item.id) === recordServiceId,
  );
  const now = new Date().toISOString();
  const savedAnswer = {
    id: 1,
    ...
    description:
      req.body?.description || "Formulario para certificaciones extranjeras",
    form: req.body?.form || {
...
// Mock base (puedes crecerlo si quieres)
const getCommitteeDates = (db) => {
  db.data.committeeDates ||= [];
  return db.data.committeeDates;
};
router.get("/committee-dates", async (req, res) => {
  try {
    const db = getConnection();
    const committeeDates = getCommitteeDates(db);

    const year = req.query.year ? Number(req.query.year) : null;
    const month = req.query.month ? Number(req.query.month) : null;

    const page = Math.max(Number(req.query.page) || 1, 1);
    const limit = Math.max(Number(req.query.limit) || 11, 1);

    let filtered = committeeDates;

    // ✅ Solo filtrar si viene year
    if (year !== null) {
      filtered = filtered.filter(item => item.year === year);
    }

    // ✅ Solo filtrar si viene month
...
router.put("/committee-dates/:id", async (req, res) => {
  try {
    const db = getConnection();
    const data = db.data;

    const id = Number(req.params.id);
    const { committeeDate } = req.body;

    if (!committeeDate) {
      return res.status(400).json({
        statusCode: 400,
        message: "committeeDate es requerido",
      });
    }

    const item = data.committeeDates.find(c => c.id === id);
    ...
    const newDate = new Date(committeeDate);
    const originalDate = new Date(item.committeeDate);
    ...
    if (
      ...
      return res.status(400).json({
        statusCode: 400,
        message: "Solo puedes modificar el día dentro del mismo mes",
      });
    }

    // ✅ Actualizar
    item.committeeDate = newDate.toISOString().split("T")[0];
...
router.post("/committee-dates/preload", async (req, res) => {
  try {
    const db = getConnection();
    const committeeDates = getCommitteeDates(db);

    const { year, createdBy, dates } = req.body;

    if (!year || !createdBy || !Array.isArray(dates)) {
      return res.status(400).json({
    ...
    const duplicated = dates.filter(d =>
      committeeDates.some(c =>
        c.year === year && c.month === d.month
      )
    );

    if (duplicated.length > 0) {
      return res.status(409).json({
        message: "Ya existen meses cargados para ese año",
        duplicatedMonths: duplicated.map(d => d.month),
      });
    }

    // 🔥 Validar duplicados dentro del request (muy importante)
    const months = dates.map(d => d.month);
    const repeated = months.filter((m, i) => months.indexOf(m) !== i);

    if (repeated.length > 0) {
      return res.status(400).json({
        message: "Meses duplicados en la petición",
        repeatedMonths: [...new Set(repeated)],
      });
    }

    let nextId =
      committeeDates.length > 0
        ? Math.max(...committeeDates.map((c) => c.id)) + 1
        : 1;

    const now = new Date().toISOString();

    const newRecords = dates.map((d) => ({
      id: nextId++,
      year,
      month: d.month,
      committeeDate: d.committeeDate,
      createdBy,
      createDate: now,
      updateDate: null,
    }));

    db.data.committeeDates.push(...newRecords);
